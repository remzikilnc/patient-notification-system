    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-x-6">
        {!isBothLoaded ? (
          ...
          <Fragment>
            <HomeStatusCard onClick={handlePatientSystemClick} status={patientsInfo?.status === "OK"} message={patientsInfo?.message} Icon={BiHealth} title="Patient System" />
            <HomeStatusCard onClick={handleNotificationSystemClick} status={notificationsInfo?.status === "OK"} message={notificationsInfo?.message} Icon={MdHealthAndSafety} title="Notification System" />
          </Fragment>
        )}
      </div>
    </div>
