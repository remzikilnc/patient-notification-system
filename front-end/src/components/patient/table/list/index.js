                                ...
                                <tr>
                                    ...
                                    {columns?.map(col => (
                                        <th
                                            key={col.id}
                                            scope="col"
                                            className={col.action ? "relative py-3.5 pl-3 pr-4 sm:pr-6" : `px-3 py-3.5 text-left text-sm font-normal truncate ${col.sortable ? "group/dropdown cursor-pointer" : ""}`}
                                             onClick={col.sortable ? () => handleSortChange(col.id) : undefined}
                                        >
                                            {col.id === "actions" || col.id === "actions-w-slug" ? (
                                                <span className="sr-only">{col.label}</span>
                                            ) : (
                                                <div
                                                    className={`flex gap-x-2 ${col.sortable ? "group/dropdown cursor-pointer" : ""} ${filters.sort[col.id] === "ASC" ? "font-semibold" : filters.sort[col.id] === "DESC" ? "font-semibold" : ""} `}>
                                                    {col.label}
                                                      {col.sortable && (
                                                            <AiOutlineArrowDown
                                ...
                                {data.length > 0 ? (
                                    data.map(item => (
                                        <tr key={item.id} className={selectedData.includes(item) ? "bg-primary/30" : undefined}>
                                            ...
                                            {columns.map(col => (
                                                <UITableCell key={col.id} type={col.type} value={item[col.id]}
                                                             item={item} colId={col.id} endpoint={col.endpoint}
                                                             text={col.text} className={col.className}/>
                                            ))}
                                        </tr>
                                    ))
                                ) : (
