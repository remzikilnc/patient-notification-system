                        ...
                        <DialogPanel transition className="w-full max-w-3xl rounded-xl bg-white p-6 duration-300 ease-out data-[closed]:transform-[scale(95%)] data-[closed]:opacity-0">
                            ...
                            <form ref={formRef} onSubmit={submit} className="mt-5">
                                <div className="grid grid-cols-1 gap-y-4 gap-x-4 border-b border-passiveBorder pb-5">
                                    ...
                                    <div className="grid md:grid-cols-2 grid-cols-1 gap-y-3 md:gap-y-0 gap-x-6">
                                        <div className="col-span-1">
                                            <UIFormLabel htmlFor="email" className="!font-medium" label="Email" />
                                            <UIFormInputText id="email" name="email" defaultValue={contact?.email} error={errors?.email} required isFocused autoComplete="email" />
                                        </div>
                                        <div className="col-span-1">
                                            <UIFormLabel htmlFor="phoneNumber" className="!font-medium" label="Phone Number" />
                                            <UIFormInputText id="phoneNumber" name="phoneNumber" defaultValue={contact?.phoneNumber} error={errors?.phoneNumber} required isFocused autoComplete="phoneNumber" />
                                        </div>
                                    </div>

                                    <div className="grid md:grid-cols-5 grid-cols-3 gap-y-3 md:gap-y-0 gap-x-6">
                                        <div className="col-span-3 md:col-span-2">
                                            <UIFormLabel htmlFor="address" className="!font-medium" label="Address" />
                                            <UIFormInputText id="address" name="address" defaultValue={contact?.address} error={errors?.address} required isFocused autoComplete="address" />
                                        </div>
                                        <div className="col-span-1">
                                            <UIFormLabel htmlFor="city" className="!font-medium" label="City" />
                                            <UIFormInputText id="city" name="city" defaultValue={contact?.city} error={errors?.city} required isFocused autoComplete="city" />
                                        </div>
                                        <div className="col-span-1">
                                            <UIFormLabel htmlFor="state" className="!font-medium" label="State" />
                                            <UIFormInputText id="state" name="state" defaultValue={contact?.state} error={errors?.state} required isFocused autoComplete="state" />
                                        </div>
                                        <div className="col-span-1">
                                            <UIFormLabel htmlFor="zip" className="!font-medium" label="Zip" />
                                            <UIFormInputText id="zip" name="zip" defaultValue={contact?.zip} error={errors?.zip} required isFocused autoComplete="zip" />
                                        </div>
                                    </div>
                                    <div className="grid md:grid-cols-2 grid-cols-1 gap-y-3 md:gap-y-0 gap-x-6">
                                        <div className="col-span-1">
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-1">
                                            <UIFormInputTextArea id="notes" name="notes" defaultValue={contact?.notes} error={errors?.notes} rows={3} desc="Write a few sentences about this contact." />
                                        </div>
                                    </div>
                                </div>
                            </form>
