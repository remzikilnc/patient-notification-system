...
const UIButtonCard = ({status, title, message, Icon, onClick}) => {
  ...
  return (
    ...
    >
      <span
        className={`${disabled ? "!bg-darkPassiveText" : ""} h-10 w-10 rounded-full absolute -top-3 -right-3 z-10 ring-8 ${
          status ? "bg-green-200 ring-green-100" : "bg-red-700 animate-ping group-hover:animate-none duration-150 transition-all ring-red-500"
        } group-hover:bg-primary group-hover:ring-white`}
      ></span>
