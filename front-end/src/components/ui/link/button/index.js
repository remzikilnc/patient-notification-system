...
export const UILinkButton = ({subMenu, activePathName, className, showDefaultIcon}) => {
    const linkClassName = classNames(
        activePathName === subMenu.activePathName ? "bg-white text-primary" : "text-white hover:text-primary transition-all delay-75 ",
    ...
    return (
        <Link href={subMenu.href} className={linkClassName}>
            {subMenu.icon ? (
                ...
                <span className={classNames(activePathName === subMenu.activePathName ? "bg-primary" : "bg-white", "mr-3 block transition-all z-10 group-hover:bg-primary duration-200 h-[1px] w-4")}></span>
            ) : null}
