    ...
    return (
        <div className="flex relative flex-wrap items-stretch w-full group">
            ...
            <div className="flex-1 w-full">
                <UIFormInputText disabled={!isChecked} defaultValue={!isChecked ? '' : defaultValue} type={type} className={`!border-l-0 !rounded-l-none !outline-0 !ring-0 group-focus-within:border-primary !duration-100 ${isChecked ? 'border-primary' : ''}  ${className}`} isFocused={isFocused} error={error} {...props} />
            </div>
        </div>
    );
};
